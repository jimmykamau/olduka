...
class ValidateEmailView(generics.GenericAPIView):
    ...
    def get(self, request):
        error_url = request.build_absolute_uri(
            settings.EMAIL_VERIFICATION_ERROR_URL)
        success_url = request.build_absolute_uri(
            settings.EMAIL_VERIFICATION_SUCCESS_URL)
        token_expired_url = error_url
        if settings.TOKEN_EXPIRED_URL:
            token_expired_url = request.build_absolute_uri(
                settings.TOKEN_EXPIRED_URL)
    ...
    def post(self, request):
        email_serializer = user_serializers.EmailValidationSerializer(
            data=request.data)
        if not email_serializer.is_valid():
            return response.Response(
                {"detail": email_serializer.errors},
                status=status.HTTP_400_BAD_REQUEST
            )
        email_address = email_serializer.validated_data['email']
